import masterData from '@/data/journals.master.json';
import { getQuartileFromPercentile } from '@/lib/scopus';
import { adaptMasterJournal } from '@/lib/journal-adapter';
import type { MasterJournalFile } from '@/types/journal-master';
...
const typedMasterData = masterData as MasterJournalFile;

export function getAllJournals() {
  return typedMasterData.journals
    .filter((journal) => journal.status?.isPublished !== false)
    .map(adaptMasterJournal)
    .map((journal) => ({
      ...journal,
      quartile:
...
export function getUniqueSubjects() {
  const allSubjects = getAllJournals().flatMap(
    (journal) => journal.subjectAreas || []
  );
