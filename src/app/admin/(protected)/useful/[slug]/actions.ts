...
import {
  updateUsefulPageContent,
  type UsefulPageWithSources
} from '@/lib/useful';
import type {LocalizedText, UsefulBlock} from '@/types/useful-page';

export type UsefulPageFormValues = {
  slug: string;
  titleRu: string;
  titleUz: string;
  titleEn: string;
...
export type UsefulPageFormState = {
  success: boolean;
  message: string;
  errors: Partial<Record<keyof UsefulPageFormValues, string>>;
  values: UsefulPageFormValues;
};

function readText(formData: FormData, key: keyof UsefulPageFormValues): string {
...
function toValues(slug: string, formData: FormData): UsefulPageFormValues {
  return {
    slug,
...
function pageToValues(page: UsefulPageWithSources): UsefulPageFormValues {
  return {
    slug: page.slug,
...
export async function updateUsefulPage(
  slug: string,
  _prevState: UsefulPageFormState,
  formData: FormData
): Promise<UsefulPageFormState> {
  ...
  if (user.role !== 'SUPERADMIN' && user.role !== 'EDITOR') {
    return {
      success: false,
      message: 'Only SUPERADMIN or EDITOR can update useful pages.',
      errors: {},
  ...
  const errors: Partial<Record<keyof UsefulPageFormValues, string>> = {};
  ...
  try {
    const saved = await updateUsefulPageContent(slug, {
    ...
    revalidatePath(`/ru/useful/${slug}`);
    revalidatePath(`/uz/useful/${slug}`);
    revalidatePath(`/en/useful/${slug}`);

    return {
      success: true,
      message: 'Useful page saved successfully.',
      errors: {},
      values: pageToValues(saved)
    };
  } catch (error) {
    return {
      success: false,
